...
function VariantDetails() {
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/api/modelvarient/getRelatedVarients?productId=${productdetails[0].productId}&cityid=${location?.value}`).then((res) => {
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async (context) => {
    ...
    if (!id.includes(".")) {
      ...
      const response = await axios.get(`${baseUrl}/api/modelvarient/getModelVarientsById?ModelVariantName=${id}&cityid=${location}`)
